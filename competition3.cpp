...
void lidarCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  ...
  outrem.setMinNeighborsInRadius (15);//18
  ...
  pass.setFilterLimits (-0.7, 300); //0.9
  //pass.setFilterLimitsNegative (true);
  pass.filter (*lidar_filtered);

  pcl::toPCLPointCloud2(*lidar_filtered,*temp_pc2);  // pointxyz to pc2
...
int main (int argc, char** argv)
{
  ...
  ros::param::get("file", file_path);
  if (pcl::io::loadPCDFile<pcl::PointXYZ> ("/home/alex/catkin_ws/src/midterm_competition3/src/downsampled.pcd", *cloud) == -1) //* load the file  
  //if (pcl::io::loadPCDFile<pcl::PointXYZ> (file_path.c_str(), *cloud) == -1) //* load the file
  {
  ...
  outrem2.setMinNeighborsInRadius (3); //<-map18 15
  ...
  // outrem2.setInputCloud(cloud);
  // outrem2.setRadiusSearch(0.8);
  // outrem2.setMinNeighborsInRadius (3);//5
  // outrem2.setKeepOrganized(true);
  // // apply filter
  // outrem2.filter (*cloud2);
  ...
  // // pass.setFilterLimits (0, 0.1);
  // pass.setFilterFieldName ("z");
  // pass.setFilterLimits (0.9, 300); //0.9
  // //pass.setFilterLimitsNegative (true);
  ...
  while(nh.ok())
  {
    ...
    if(get_lidar_data == 1){  // && get_imu_data == true
      ...
      distance_square = (processing_gps_data.point.x - odom.pose.pose.position.x)*(processing_gps_data.point.x - odom.pose.pose.position.x)+
                        (processing_gps_data.point.y - odom.pose.pose.position.y)*(processing_gps_data.point.y - odom.pose.pose.position.y)+
                        (processing_gps_data.point.z - odom.pose.pose.position.z)*(processing_gps_data.point.z - odom.pose.pose.position.z);
      std::cout << "odom/gps difference: " << distance_square << std::endl;

      if(distance_square>18){
        std::cout << "bad match, use GPS data" << std::endl;
        ...
        updated_guess(0,3) = odom.pose.pose.position.x;
        updated_guess(1,3) = odom.pose.pose.position.y;
        updated_guess(2,3) = odom.pose.pose.position.z;
        fit_flag = false;
      }
      else{
