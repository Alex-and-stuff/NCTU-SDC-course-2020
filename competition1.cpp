...
int loop_counts = 0;
int lidar_count = 0;
float yaw = 90.35;//2.4; //90.3; //-2.2370340344819;
...
int main (int argc, char** argv)
{
  ...
  // Initialize ROS
  ros::init (argc, argv, "competition1_node");
  ros::NodeHandle nh;
  ...
  //if (pcl::io::loadPCDFile<pcl::PointXYZ> ("/home/alex/catkin_ws/src/midterm_competition1/src/itri_map.pcd", *cloud) == -1) //* load the file
  ...
  std::cout << "Loaded "
            ...
            << " data points from itri_map.pcd with the following fields: "
            << std::endl;
  ...
  while(nh.ok())
  {
    ...
    try{
      transformStamped = tfBuffer.lookupTransform("base_link", "velodyne", ros::Time(0));
      // loop_rate.sleep();
    }
    ...
    if(get_lidar_data == 1){  // && get_imu_data == true
      std::cout << "get lidar data true... inside loop" << std::endl;
